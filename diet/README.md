...
| 体型 | 身長・股下・体重・骨格筋・体脂肪（% か kg）、除脂肪量・筋肉量・部位別筋肉量（体幹部・右腕・左腕・右足・左足）から今の体をイラストで表示。部位別の筋肉を身長あたりの標準と比較し左右差を指摘。スライダーと一覧で「何kgならどう見えるか」を予測（筋トレあり/なし切替） |
...
- 体脂肪率は **体脂肪率 > 除脂肪量 > 筋肉量（÷0.95 で除脂肪量に換算）> 骨格筋率からの換算 > BMI式（Deurenberg）** の優先順で使う。
  除脂肪量・筋肉量は体重とセットの値なので、最新の体重と同じ日の測定のときだけ使う。
- 骨格筋・体脂肪は % でも kg でも入力できる（入力欄の単位ボタンで切替・端末に記憶）。保存は常に %（kg ÷ その日の体重）。
- 部位別筋肉量は「kg ÷ 身長m²」を標準（男性: 体幹 9.35・腕 1.08・脚 3.45、女性: 7.8・0.76・2.9 = FFMI 男性20・女性16相当）と比べ、
  標準比 +15% ごとに FFMI +3 相当だけ、その部位（肩・胸 / 左右の腕 / 左右の脚）を太く描く。図は正面から見た向き（本人の右は画面の左）。
- 股下: 股下 ÷ 身長 の比で、胴と脚の縦の長さを伸縮する（基準 0.455）。
